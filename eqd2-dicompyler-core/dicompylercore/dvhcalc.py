...
# This file has been modified for biological recalculation using EQD2 by Cornelio Salinas
...
def get_dvh(eqd2correction, structure,
            dose,
            roi,
            limit=None,
            calculate_full_volume=True,
            use_structure_extents=False,
            interpolation_resolution=None, #THIS IS 8TH PARAMETER
            interpolation_segments_between_planes=0,
            thickness=None, #THIS IS 10TH PARAMETER
            memmap_rtdose=False,
            callback=None):
    ...
    return dvh.DVH(counts=calcdvh.histogram,
                   bins=( (np.arange(0, 2)*eqd2correction) if (calcdvh.histogram.size == 1) else
                         np.arange(0, calcdvh.histogram.size + 1)*eqd2correction / 100),
